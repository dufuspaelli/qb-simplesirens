...
RegisterCommand("sirenDefault", function()
    if isSirenChange(sirenNormal) then
        playSiren(sirenNormal)
    end 
end)

RegisterCommand("sirenAlt", function()
    if isSirenChange(sirenAlt) then 
        playSiren(sirenAlt)
    end
end)

RegisterCommand("sirenWarn", function()
    if isSirenChange(sirenWarn) then 
        playSiren(sirenWarn)
    end
end)

RegisterCommand("sirenHorn", function()
    if isSirenChange(sirenHorn) then 
        playSiren(sirenHorn)
    end
end)
    ...
    while true do 
        for carNet, val in pairs(sirenList) do
            if NetworkDoesNetworkIdExist(carNet) then 
                ...
                local currentSiren = Entity(car).state.currentSiren or "muted"
                local sirenChange = currentSiren ~= val.siren or false
                local sirenOn = Entity(car).state.sirenOn == nil and IsVehicleSirenOn(car) or false
                if muted then 
                    StopSound(Entity(car).state.soundId)
                    ReleaseSoundId(Entity(car).state.soundId)
